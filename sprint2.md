...
![script link](/CarDexBackend/scripts/run-regression-tests.sh)
...
![Backend Report coverage](./assets/backend_coverge_report.png)

The things we have excluded from coverage are migration beacuse it is autogenerated. Extensions are for debugging purposes and do not really need tests. Global Exception Handler are for nicer way to send specific response to client.
